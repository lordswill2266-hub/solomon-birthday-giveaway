...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === 'markPaid') {
      ...
      const r = await fetch(`${url}/rest/v1/winner_payouts?id=eq.${encodeURIComponent(payoutId)}`, { method: 'PATCH', headers: h, body: JSON.stringify({ payment_status: 'paid', paid_at: new Date().toISOString() }) });
      if (!r.ok) throw new Error(await r.text());
      return respond(res, 200, { success: true });
    }

    if (action === 'resetTestData') {
      const a = await fetch(`${url}/rest/v1/winner_payouts?id=not.is.null`, { method: 'DELETE', headers: h });
      if (!a.ok) throw new Error(await a.text());
      const b = await fetch(`${url}/rest/v1/giveaway_entries?id=not.is.null`, { method: 'DELETE', headers: h });
      if (!b.ok) throw new Error(await b.text());
      const c = await fetch(`${url}/rest/v1/giveaway_config?id=eq.1`, { method: 'PATCH', headers: h, body: JSON.stringify({ winner_count: 0, is_open: true }) });
    ...
    const entriesResp = await fetch(`${url}/rest/v1/giveaway_entries?is_winner=eq.true&select=id,claim_code,created_at,prize_claimed&order=created_at.desc`, { headers: h });
    if (!entriesResp.ok) throw new Error(await entriesResp.text());
    const entries = await entriesResp.json();
    const payoutByEntry = Object.fromEntries((payouts || []).map(p => [p.entry_id, p]));

    const winners = (entries || []).map(e => {
      const p = payoutByEntry[e.id];
      return {
        entryId: e.id,
        claimCode: e.claim_code,
        wonAt: e.created_at,
        detailsSubmitted: !!p,
        status: p ? p.payment_status : 'awaiting_details',
        payoutId: p?.id || null,
        accountName: p?.account_name || null,
        bankName: p?.bank_name || null,
        accountNumber: p ? decrypt(p.account_number_encrypted) : null,
    ...
    return respond(res, 200, { payouts: winners.filter(w => w.detailsSubmitted), winners, config });
  } catch (error) {
